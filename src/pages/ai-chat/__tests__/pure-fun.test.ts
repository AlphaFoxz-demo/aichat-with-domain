...
import { genId, readableSize } from '../domain/common';
...
describe('conversation', () => {
  ...
  it('createMessage', () => {
    const conversation = createConversation('1');
    conversation.createMessage({ creator: 'User', text: ['你好'] });
  ...
  it('fully http response', () => {
    const conversation = createConversation('1');
    conversation.createMessage({ creator: 'DeepSeek', text: [] });
    ...
    conversation.onMessage('你好！');
    expect(conversation.content[0].text[0]).toBe('你好！');
    conversation.finishNode();
    expect(conversation.content[0].text.length).toBe(1);
    conversation.startNode();
    expect(conversation.content[0].text[1]).toBe('');
    conversation.onMessage('很高兴见到你！😊');
    expect(conversation.content[0].text[1]).toBe('很高兴见到你！😊');
    conversation.finishNode();
